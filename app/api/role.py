...
@router.get("/get_role", tags=["Role"], dependencies=[Depends(PermissionsValidator(["app-admin:role"]))])
async def get_role():
    response = await get_all_roles()
    return {"response": response}

@router.post("/create_role", tags=["Role"], dependencies=[Depends(PermissionsValidator(["admin:update-role"]))])
...
@router.post("/assign-permission/{role_id}", response_model=Role.MessageResponse, tags=["Role"], dependencies=[Depends(PermissionsValidator(["admin:update-role"]))])
...
@router.post("/create_role_and_assign_permission", tags=["Role"], dependencies=[Depends(PermissionsValidator(["admin:update-role"]))])
...
@router.post("/list-permissions", tags=["Role"], dependencies=[Depends(PermissionsValidator(["app-admin:role"]))])
